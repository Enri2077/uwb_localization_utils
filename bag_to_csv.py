...
def bag_to_csv(bag_path: str) -> None:
    ...
    with AnyReader([Path(bag_path)]) as reader:
        topic_field_data = defaultdict(list)
        connections = [x for x in reader.connections if x.topic == "/UWB/Ranges"]
        for connection, timestamp, rawdata in reader.messages(connections=connections):
            msg = reader.deserialize(rawdata, connection.msgtype)
            topic_field_data['t'].append(timestamp/1E9)
            topic_field_data['r'].append(msg.range)

        df = pd.DataFrame(topic_field_data)
        df.to_csv(path.join(bag_path, f"ranges.csv"))


def main():
    bag_to_csv(path.expanduser('~/uwb_logs/laser_tracker/uwb_laser_tracker_bags/rosbag2_2024_07_26-12_28_39'))
